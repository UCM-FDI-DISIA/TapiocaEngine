...
private:
	...
	*/ 
	...
	/*
	* TO DO: 
	*	- Hacer un enum de eventos mapeables
	* 
	*	- Mapear varios botones a un evento con un unordered_multimap<evento,botón> para identificarlos y luego
	*	  hacer un array de booleanos usando el enum de eventos para comprobar si dichos eventos están activos
		  (por ejemplo, mappedEvent[me_jump] = ( )
	*		Problema: El estado del teclado está representado con un uint8_t*, el de los botones del
	*				  con un vector<bool>, y el de los ejes del mando con pair<Vector2, Vector2>
	*				  => Se puede interpretar el estado del 
	* 
	*	- Comprobar 
	*/
	// Comprobar si el evento está sucediendo o no
