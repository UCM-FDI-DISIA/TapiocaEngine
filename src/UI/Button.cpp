...
Button::Button(const ButtonOptions& options) {
    Button(options.name, options.position, options.text, options.onClick, options.constSize, options.padding, options.textFont,
           options.textColor, options.normalColor, options.hoverColor, options.activeColor, options.canCloseWindow,
           options.windowFlags);
}

Button::Button(const std::string& name, const ImVec2& position, const std::string& text, std::function<void()> onClick, const ImVec2& constSize,
    ...
    : name(name), position(position), text(text), onClick(std::move(onClick)), constSize(constSize), padding(padding),
