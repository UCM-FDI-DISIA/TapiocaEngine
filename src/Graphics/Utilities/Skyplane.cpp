...
Skyplane::Skyplane(Ogre::SceneManager* const scnMngr, RenderNode* const node, Ogre::MeshManager* const mshMgr,
    ...
    : RenderObject(node, scnMngr), sceneManager(scnMngr), material(materialName), rkNormal(rkNormal),
      fConstant(fConstant), scale(scale), tiling(tiling), drawFirst(drawFirst), bow(bow), xSegments(xsegments),
      ySegments(ysegments) {
