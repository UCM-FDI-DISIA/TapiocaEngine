...
Skybox::Skybox(Ogre::SceneManager* scnMgr, Node* node, std::string texture) : RenderObject(node, scnMgr), texture(texture) {
    scnM = scnMgr;
    scnMgr->setSkyBox(true, texture);
}

Skybox::Skybox(Ogre::SceneManager* scnMgr, Node* node, std::string texture, int distC)
    : RenderObject(node, scnMgr), texture(texture) , distC(distC) {
    scnM = scnMgr;
    scnMgr->setSkyBox(true, texture, distC);
}

Skybox::Skybox(Ogre::SceneManager* scnMgr, Node* node, std::string texture, int distC, bool orderC)
